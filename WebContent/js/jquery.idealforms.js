...
$.idealforms.errors = {
  ...
  required: 'Here is a required.',
  number: '必须是数字.',
  studentId: 'Must have 10 numbers',
  digits: '必须是唯一的数字.',
  name: 'There must be at least 3 characters long, and can contain only letters.',
  username: 'Must be between 32 and 4 characters long and begin with a letter.',
  pass: 'Must be at least 6 characters long, and must contain at least one number, one capital letter and one lowercase letter.',
  strongpass: '必须至少为8个字符长，至少包含一个大写字母和一个小写字母和一个数字或特殊字符.',
  email: '必须是一个有效的e -mail地址. <em>(e.g. user@gmail.com)</em>',
  phone: 'Must be a valid China cellphone number. <em>(e.g. 13392632235)</em>',
...
var getFilters = function() {
  ...
  var filters = {
    ...
    studentId: {
      regex: /^\d{10}$/,
      error: $.idealforms.errors.studentId
    },
    zip: {
      regex: /^\d{5}$|^\d{5}-\d{4}$/,
